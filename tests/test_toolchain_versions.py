...
def test_version_requirement_uses_selected_board_chip(monkeypatch):
    ...
    resolved["toolchain"]["detect_source"] = None
    assert resolved["toolchain"]["chip_version_constraints"] == ["2024.1+"]
    ...
    other["toolchain"]["detect_source"] = None
    assert other["toolchain"]["chip_version_constraints"] == ["*"]
...
def test_detected_version_must_agree_with_configured_version(monkeypatch):
    ...
    tc["chip_version_constraints"] = ["*"]
    assert tc["configured_version"] == "2023.2" and tc["detected_install_version"] == "2024.1"
...
def test_detected_version_is_checked_against_chip_requirement(monkeypatch):
    ...
    tc["chip_version_constraints"] = ["2024.1+"]
    with pytest.raises(config.init.ConfigError, match="does not meet chip constraint"):
        config.init.require_toolchain_version(tc)
    tc["detected_install_version"] = "2024.1"
    config.init.require_toolchain_version(tc)
...
def test_invalid_chip_variant_constraint_cannot_hide_behind_wildcard():
    tc = {"id": "vivado", "chip_version_constraints": ["*", "2024.2-2024.1"]}
...
def test_synthesis_rejects_version_conflict_before_output(tmp_path, monkeypatch):
    ...
    resolved["toolchain"]["detected_install_version"] = "wrong-version"
    resolved["toolchain"]["detect_source"] = "test"
