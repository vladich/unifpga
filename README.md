...
2. **The board and the rig.** A board is a YAML pinmap in
   `config/boards/<producer>/<family>/<id>.yml` and a layout
   (`config/layouts/<board>.yml`: its connectors and on-board parts). A rig is
...
| `config/layouts/<board>.yml`, `config/modules/<id>.yml`, `config/connectors.yml` | Boards as rigs' canvases: connectors and on-board parts (generated by `./unifpga layout draft <board>` from the pinmap and the board-sources registry), add-on module pinouts, connector types. |
...
- **A new board**: write its pinmap under
  ...
  draft its layout (`./unifpga layout draft <id>`) and write a rig
...
│   ├── layouts/<board>.yml    # boards as drawn: connectors and on-board parts
