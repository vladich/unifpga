...
endgenerate
    ...
    always_ff @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            in[0] <=   (mic_sample >>> 1);                //  0.5
            in[1] <=   (mic_sample >>> 1) + (mic_sample >>> 3);  //  0.625
            in[2] <=    mic_sample        - (mic_sample >>> 2);  //  0.75
            in[3] <=    mic_sample        - (mic_sample >>> 3);  //  0.875
            in[4] <=    mic_sample;                       //  1.0
            in[5] <= - (mic_sample >>> 1);                // -0.5
            in[6] <= - (mic_sample >>> 1) - (mic_sample >>> 3);  // -0.625
            in[7] <= -  mic_sample        + (mic_sample >>> 2);  // -0.75
            in[8] <= -  mic_sample        + (mic_sample >>> 3);  // -0.875
            in[9] <= -  mic_sample;                       // -1.0
        end
    end
