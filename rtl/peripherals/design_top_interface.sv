...
# (
    ...
    // ---- Optional capabilities: declare them only when the design uses them
    // (the generated top connects what the design declares) -------------------
    ...
    parameter int w_y = (screen_height > 1) ? $clog2(screen_height) : 1,
...
(
    ...
    // ---- Switches (active-high — codegen normalizes polarity) ---------------
    input        [w_sw     - 1 : 0]   sw,

    // ---- Buttons (active-high — codegen normalizes polarity) ----------------
    input        [w_btn    - 1 : 0]   btn,

    // ---- LEDs (active-high) -------------------------------------------------
    output logic [w_led    - 1 : 0]   led,

    // ---- 7-segment display (shared segment lines + digit-select): 8 = 7
    // segments (a..g) + decimal point; the user multiplexes across digits -----
    ...
    // ---- Pixel display: (x, y) drives in from the screen provider; the user
    // computes the pixel colour at that coordinate and drives red/green/blue
    // back out ----------------------------------------------------------------
    input        [w_x      - 1 : 0]   x,
    input        [w_y      - 1 : 0]   y,
    ...
    // ---- Small display (optional): the driver fetches the pixel at (sd_x,
    // sd_y) one clock after presenting it -------------------------------------
    input        [w_sd_x   - 1 : 0]   sd_x,
    input        [w_sd_y   - 1 : 0]   sd_y,
    output logic [w_sd_pixel- 1 : 0]  sd_pixel,

    // ---- Character display (optional): the character code at (txt_col,
    // txt_row), fetched one clock after the position --------------------------
    input        [w_txt_col- 1 : 0]   txt_col,
    input        [w_txt_row- 1 : 0]   txt_row,
    output logic [         7 : 0]     txt_char,

    // ---- Actuators, each with an on/off bit and an 8-bit level (position /
    // duty). ------------------------------------------------------------------
    output logic [w_act    - 1 : 0]   act_on,
    output logic [w_act_level- 1 : 0] act_level,

    // ---- Keyboard (optional): one clock of kbd_valid per key event; kbd_key
    // is the key's USB HID usage code (A = 8'h04, Enter = 8'h28), kbd_down 1
    // pressed -----------------------------------------------------------------
    input                             kbd_valid,
    input        [         7 : 0]     kbd_key,
    input                             kbd_down,
    ...
    // adc_value is the 12-bit code of channel adc_channel (mV = value *
    // adc_mv / 4096) ----------------------------------------------------------
    input                             adc_valid,
    ...
    // (NEC: the remote's address, the key's command byte), ir_repeat while
    // held --------------------------------------------------------------------
    input                             ir_valid,
    ...
    // ---- Memory (optional): the board's RAM, 2^w_mem_addr words of
    // mem_bytes bytes; a request (mem_req; mem_we and mem_be for a write)
    // while mem_ready, one clock of mem_ack when it is done, mem_rdata a
    // read's word -------------------------------------------------------------
    ...
    // st_block while st_ready; then 512 bytes, each one clock of st_valid
    // with st_data, and st_done or st_error; st_present while a card answers --
    output logic                      st_req,
