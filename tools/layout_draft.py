...
as attaches. draft() builds one from:
  ...
  * the board's registry entry (tools/board_sources.py): a header whose facts
...
def draft(board_id):
    ...
    connectors_def = su.read_connectors()
    ...
    return {"board": board_id, "verified": verified, "generated": True, "connectors": connectors, "onboard": onboard}
...
def emit(layout):
    ...
    out = ["# Layout of {} generated by tools/layout_draft.py from its pinmap, its".format(layout["board"]),
           "# configurations and config/board_sources/{}.yml; regenerate it with".format(layout["board"]),
           ...
           "  generated: true", "", "  connectors:" + ("" if layout["connectors"] else " []")]
